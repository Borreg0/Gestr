...
class Empleados_page(QWidget):
    def __init__(self, parent = None):
        ...
        self.columnasTableWidget = ["Nombre", "Departamento", "Email", "Teléfono", "Rol"]
...
#
    ...
    def llenarTabla(self):
        try:
            ...
            self.tableWidget.setColumnCount(len(Empleados[0].keys())-1)
            self.tableWidget.setRowCount(len(Empleados))
            ...
            for Empleado in Empleados:
                ...
                self.tableWidget.setCellWidget(fila,0,self.botonesEditarEliminar(Empleado_id,Empleado,fila))
                self.tableWidget.setColumnWidth(0,85)
                
                #datos Empleado
                #self.tableWidget.setItem(fila,1,QTableWidgetItem(str(Empleado_id)))
                
                self.tableWidget.setItem(fila,1,QTableWidgetItem(nombre))
                self.tableWidget.setColumnWidth(1,180)
                
                self.tableWidget.setItem(fila,2,QTableWidgetItem(departamento))
                self.tableWidget.setColumnWidth(2,150)
                
                self.tableWidget.setItem(fila,3,QTableWidgetItem(email))
                self.tableWidget.setColumnWidth(3,270)

                self.tableWidget.setItem(fila,4,QTableWidgetItem(telefono))
                self.tableWidget.setColumnWidth(4,150)
                
                self.tableWidget.setItem(fila,5,QTableWidgetItem(rol))
    ...
    def botonEditarEliminar_pulsado(self, fila):
        ...
        self.tableWidget.setCurrentCell(fila, 3)  
